...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    pos_mask = (x > 0)
    postives = x[pos_mask]
    pos_sum = torch.sum(postives).item()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return pos_sum
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # pass
    dim = 1
    min_indices = torch.argmin(x, dim=dim)
    # print(min_indices)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu = x.sum(dim=0)  # (N)
    mu = mu / M  # (N)
    sigma = ((x - mu)**2).sum(dim=0)  # (N)
    sigma = torch.sqrt(sigma / (M-1))  # (N)
